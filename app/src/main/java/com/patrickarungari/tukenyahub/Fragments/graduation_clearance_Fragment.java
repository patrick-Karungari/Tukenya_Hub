...
public class graduation_clearance_Fragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                arrayAdapter = ArrayAdapter.createFromResource(Objects.requireNonNull(getActivity()),
                        R.array.term_of_deferment, android.R.layout.simple_spinner_item);
                arrayAdapter2 = ArrayAdapter.createFromResource(getActivity(),
                        R.array.term_of_deferment, android.R.layout.simple_spinner_item);
                Objects.requireNonNull(graduation_clearance_Fragment.this.getActivity()).runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        initUi(view);
                    }
